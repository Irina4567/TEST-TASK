...
<body>
    ...
    <div class="wrapper">
        ...
        <? 
            ...
            function item_list ($url, $name, $list, $class_item, $class_link, $parent_tag) {
                $sub_list = '';
                foreach ($list as $sub_item)
                    $sub_list .= '<li class="'.$class_item.'"><a class="'.$class_link.'" href="'.$sub_item->URL.'">'.$sub_item->NAME.'</a></li>';
                if ( $parent_tag == 'ul') {
                    return generate_list($url, $name, $sub_list, "submenu-nav", "has-submenu", '', '', $parent_tag);
                } else if ($parent_tag == 'li') {
                    return generate_list($url, $name, $sub_list, "", "mega-menu-item", 'mega-title', '', $parent_tag);
                } else if ($parent_tag == 'ul-mobile') {
                    return generate_list($url, $name, $sub_list, "nav-items nav-expand-content", "nav-item nav-expand", 'nav-link nav-expand-link', '',$parent_tag);
                } else if ($parent_tag == 'li-mobile') {
                    return generate_list($url, $name, $sub_list, "", "nav-item nav-expand", 'nav-link nav-expand-link','nav-items nav-expand-content', $parent_tag);
                }
            }

            function generate_list ($url, $name, $sub_list, $class_item_menu, $class_item_sub, $class_link, $class_item_sub_menu, $parent_tag) {
                if ($parent_tag == 'ul' || $parent_tag == 'ul-mobile') {
                    ...
                    return $list;
                } else if ($parent_tag == 'li' || $parent_tag == 'li-mobile') {
                    $sub_list = '<li class="'.$class_item_sub.'"><a class="'.$class_link.'" href="'.$url.'">'.$name.'</a><ul class="'.$class_item_sub_menu.'">'.$sub_list.'</ul></li>';   
                    return $sub_list;
                } 
            }
                        ...
                        <?
                            ...
                            foreach ($struct as $menu_item) {
                                if ($menu_item->tip == 'once') {
                                    echo item_once ($menu_item->URL, $menu_item->NAME, "nav-item", "nav-link");
                                }

                                if ($menu_item->tip == 'list') {
                                    echo item_list ($menu_item->URL, $menu_item->NAME, $menu_item->lists, 'nav-item', 'nav-link', 'ul-mobile');
                                }
                                ...
                                if ($menu_item->tip == 'catalog') {
                                    ...
                                    {
                                        $sub_list .= item_list ($sub_item->URL, $sub_item->NAME, $sub_item->PODKATEGORIA, 'nav-item', 'nav-link', 'li-mobile');
                                    }
                                    echo generate_catalog ($menu_item->URL, $menu_item->NAME, $sub_list, "nav-items nav-expand-content", "nav-item nav-expand", "nav-link nav-expand-link");
                                }
                            }
                        ?>
                    </ul>
